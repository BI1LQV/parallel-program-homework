...
int main(int argc, char **argv)
{
	...
	// free(A.columnindex);
	// free(A.value);
	VALUE_TYPE *A_val_line[60000];
	int *A_idx_line[60000];
	int A_nnz_line[60000];
	for (int row = 0; row < 60000; row++)
	{
		A_nnz_line[row] = A.rowpointer[row + 1] - A.rowpointer[row];
		A_val_line[row] = (VALUE_TYPE *)malloc(sizeof(VALUE_TYPE) * 1000);
		A_idx_line[row] = (int *)malloc(sizeof(int) * 1000);
		memcpy(A_val_line[row], A.value + A.rowpointer[row], A_nnz_line[row] * sizeof(VALUE_TYPE));
		memcpy(A_idx_line[row], A.columnindex + A.rowpointer[row], A_nnz_line[row] * sizeof(int));
		// printf("%d\n", A_nnz_line[row] * sizeof(VALUE_TYPE));
		// for (int t = 0; t < A_nnz_line[row]; t++)
		// {
		// 	printf("%d ", A.columnindex[row]);
		// 	return 0;
		// 	A_idx_line[row][t] -= A_idx_line[row][0];
		// }
	}
	for (int s = 0; s < A_nnz_line[0]; s++)
	{
		printf("%d ", A_idx_line[0][s]);
	}
	return 0;
